...
public struct PollConfig has drop, store {
    //setting fields
...
public struct PollOption has drop, store {
    id: u64, //option index
    name: String,
...
fun init(otw: POLL, ctx: &mut TxContext) {
    ...
    let registery = PollRegistery {
        id: object::new(ctx),
        owner: ctx.sender(),
        polls: table::new<u64, ID>(ctx),
        next_poll_id: 0,
    };
...
public fun createCreatePollRequest(
    ...
    if (!option_images.is_empty()) {
        assert!(option_images.length() == option_names.length(), EUnequalLength);
    };
    ...
    let mut i: u64 = 0;
    while (i < len) {
        let image_url = if (i < option_images.length()) { *option_images.borrow(i) } else {
            option::none<String>()
        };
        let caption = if (i < option_captions.length()) { *option_captions.borrow(i) } else {
            option::none<String>()
        };
    ...
    CreatePollRequest {
        ...
        poll_config: setConfiguration(poll_config_bools),
    }
}

public fun createVoteTicket(
    version: &poll::version::Version,
    poll: &mut Poll,
    option: u64,
    owner: address,
...
public fun create_poll(
    ...
    //assert!();
    let CreatePollRequest { title, description, thumbnail_url, duration, options, poll_config } =
        createPollRequest; //input validation done on request constructor
...
public(package) fun create_poll_registery_for_testing(ctx: &mut TxContext) {
    transfer::share_object(PollRegistery {
        id: object::new(ctx),
        owner: ctx.sender(),
        polls: table::new<u64, ID>(ctx),
        next_poll_id: 0,
...
#[test_only]
public(package) fun poll_fields(
    self: &mut Poll,
): (&u64, &String, &mut option::Option<String>, &address, &u64, &u64, &bool) {
    (
        &self.poll_id,
        &self.title,
...
#[test_only]
public(package) fun poll_tables(
    self: &Poll,
): (&table::Table<u64, u64>, &table::Table<address, vector<u64>>, &table::Table<ID, vector<u64>>) {
